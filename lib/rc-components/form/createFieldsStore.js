...
var _initialiseProps = function _initialiseProps() {
  ...
  this.isModifiedField = function (name) {
    var value = _this5.getFieldValue(name);
    var fieldMeta = _this5.getFieldMeta(name);
    var initialValue = fieldMeta && fieldMeta.initialValue;
    if (!value && !initialValue) {
      return false;
    }
    if (!(0, _isEqual2['default'])(value, initialValue)) {
      return true;
    }
    return false;
  };

  this.isModifiedFields = function (names) {
    var i = void 0;
    var length = void 0;
    var fieldNames = names ? _this5.getValidFieldsFullName(names) : _this5.getValidFieldsName();
    if (fieldNames && Array.isArray(fieldNames)) {
      for (i = 0, length = fieldNames.length; i < length; i++) {
        if (_this5.isModifiedField(fieldNames[i])) {
          return true;
        }
      }
    }
